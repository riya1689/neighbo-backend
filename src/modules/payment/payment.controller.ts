...
export const initiatePayment = async (
  ...
  try {
    ...
    // 🔄 CHANGE: Now uses your new credentials from .env
...
export const paymentSuccess = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    // 🔄 CHANGE: Extracting val_id for the "More Validation" step
    const { tran_id, val_id } = req.body;

    console.log(`Payment success callback for tran_id: ${tran_id}. Starting server-side validation...`);

    // 🔄 CHANGE: New Validation Step using your Validation API
    const sslcz = new SSLCommerzPayment(SSL_STORE_ID, SSL_STORE_PASS, SSL_IS_SANDBOX);
    const validationResponse = await sslcz.validate({ val_id });

    // 🔄 CHANGE: Now checking validationResponse instead of raw req.body status
    if (validationResponse.status !== "VALID" && validationResponse.status !== "VALIDATED") {
      console.error(`🚨 Payment Validation Failed for ${tran_id}:`, validationResponse.status);
      
      // Mark as failed in database if validation fails
    ...
    // 🔄 CHANGE: Using bank_tran_id from verified response
